...
function initMap() {
    ...
    var searchAddress = new ymaps.SuggestView('SearchAddress');
    searchAddress.events.add("select", function (e) {
        ymaps.geocode($('#SearchAddress').val(), {
            results: 1
        }).then(function (res) {
    ...
    searchAddress.state.events.add('change', function () {
        var Items = searchAddress.state.get('items');
        for (var i = 0; i < Items.length; ++i)
            Items[i].value = Items[i].value.replace("Россия, ", "");
    });
    ...
    Marker = new ymaps.Placemark(
        [0, 0], {
            ...
            iconImageClipRect: [[0, 0], [48, 48]],
            iconImageHref: '/Resources/Images/Icon/MapMarker48x48.png',
            iconImageSize: [48, 48],
    ...
    function GoCenter() {
        if (position === null) setTimeout(GoCenter, 10);
        else {
            Marker.geometry.setCoordinates(position);

            map.geoObjects.add(Marker);

            map.setCenter(position, 16, { duration: 300 });
        }
    }
    ...
    function getAddress(Coordinates) {
        ymaps.geocode(Coordinates).then(function (res) {
            ...
            var formGroup = $(map.container._parentElement)
                .closest(".form-group");

            formGroup.find("[name='Address']")
                .val(firstMarker.getAddressLine());

            formGroup.find("[name='Coordinates']")
                .val(JSON.stringify(Coordinates));
        });
    }
}
